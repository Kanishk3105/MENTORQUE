...
export default function AdminDashboard() {
  ...
  const loadUsers = useCallback(async () => {
    try {
      const [u, m, ct] = await Promise.all([adminApi.listUsers(), adminApi.listMentors(), adminApi.listCallTypes()]);
      setUsers(u);
      setMentors(m);
      setCallTypes(ct);
      setScheduleCallType((prev) => prev || ct[0]?.key || "");
    } catch (e) {
  ...
  const checkOverlap = useCallback(async () => {
    if (!selectedUser?.id || !selectedMentor?.id) {
      setOverlapSlots([]);
      return;
    }
    try {
      const { overlap } = await adminApi.getOverlappingSlots(selectedUser.id, selectedMentor.id, 21);
  ...
  const handleScheduleMeeting = async (e) => {
    ...
    if (!selectedUser || !selectedMentor) {
      setScheduleInlineError("Select both a user and a mentor above before scheduling.");
      return;
    }
    if (!scheduleDate) {
    ...
    try {
      await adminApi.scheduleMeeting({
        ...
        startTime: scheduleStartIso,
        endTime: scheduleEndIso,
        userId: selectedUser?.id || null,
        mentorId: selectedMentor?.id || null,
        callType: scheduleCallType || null,
        meetingLink: scheduleMeetingLink.trim() || null,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-row items-start gap-4 w-full">
        ...
        >
          ...
          <div className="flex items-center justify-between gap-3 rounded-lg bg-slate-900/60 border border-slate-800 px-3 py-2">
            <button
              type="button"
              onClick={prevWeek}
            ...
            <span className="text-sm font-medium text-slate-200">{displayWeekInfo.weekLabel}</span>
            <button
              type="button"
              onClick={nextWeek}
              className="rounded-md p-1.5 text-slate-400 hover:bg-white/[0.06] hover:text-white transition-colors"
                          ...
                          <tr key={key} className={`${rowBg} border-b border-slate-800/80`}>
                            ...
                            <td className="py-4 px-4 text-sm text-slate-100 align-top">
                              {commonIntervals.length === 0 ? (
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {commonIntervals.map(({ startHm, endHm }) => {
            ...
            {selectedUser && selectedMentor && (
              <p className="text-xs text-slate-400 mb-3 -mt-2">
                {overlapSlots.length === 0
                  ? "No overlapping availability in the next 3 weeks."
                  : `${overlapSlots.length} overlapping slot${overlapSlots.length !== 1 ? "s" : ""} in the next 3 weeks.`}
              </p>
            )}
            <form onSubmit={handleScheduleMeeting} className="space-y-3 flex-1 flex flex-col">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setScheduleCallType(e.target.value)}
                  className="w-full box-border rounded-lg bg-slate-950 border border-slate-800 text-white px-4 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
                  {callTypes.map((ct) => (
              ...
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-1">Meeting link (optional)</label>
                <input
                  type="url"
      ...
      {activeMeeting && (
        ...
        >
          ...
          >
            ...
            <div className="mb-4">
              <p className="text-xs font-semibold text-slate-400 mb-1">Meeting link</p>
              {activeMeeting.meetingLink ? (
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const lines = [
                        ...
                        `Video call link: ${activeMeeting.meetingLink || "No link added"}`,
                      ].join("\n");
                ...
                <p className="text-xs text-slate-500">No link added</p>
              )}
            </div>
